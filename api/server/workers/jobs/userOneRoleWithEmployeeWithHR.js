...
const writeResultFile = async filters => {
  let route = path.join(__dirname, 'Files/userOneRoleWithEmployeeWithHR.json');
...
const processJob = async () => {
  queue.process(queueName, concurrency, async (job, done) => {
    try {
      ...
      console.log('userOneRoleWithEmployeeWithHR... 25%');
      job.progress(25);

      const userOneRoleWithEmployeeWithHR = await userWithOneRolRefencesOnTablesAndExitsReferenceOnHR(
        users
      );

      console.log('userOneRoleWithEmployeeWithHR... 75%');
      job.progress(75);

      const result = [
        {
          type: 'userOneRoleWithEmployeeWithHR',
          data: userOneRoleWithEmployeeWithHR,
          count: userOneRoleWithEmployeeWithHR.length
        }
      ];

      await writeResultFile(result);
      console.log('userOneRoleWithEmployeeWithHR... 100%');
