    ...
    try {
      ...
      for (let i = 0; i < users.length; i++) {
        for (let k = 0; k < users[i].user_roles.length; k++) {
          if (users[i].user_roles[k].cd_entity === 'employee') {
          ...
          } else if (users[i].user_roles[k].cd_entity === 'applicant') {
            let applicants = await applicantRepository.getApplicantbyEntityId(
              users[i].user_roles[k].id_entity
            );

            users[i].user_roles[k].applicants = [];

            if (applicants) {
              users[i].user_roles[k].applicants = applicants;
            }
          }
        }
        console.log(
